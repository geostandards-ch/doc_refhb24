...
Ein Referenzsystem-Modell (`REFSYSTEM MODEL`) soll nebst den Definitionen eines Typenmodells nur Themen und Klassen deklarieren, die im Bezug zu Erweiterungen der vordefinierten Klassen `AXIS` bzw. `REFSYSTEM` stehen (vgl. <<_referenzsysteme>>). Die Einhaltung dieser Regel kann durch die Sprache nicht erzwungen werden. Es ist Sache des Anwenders, sich daran zu halten.

Ein Signaturenmodell (`SYMBOLOGY MODEL`) soll nebst den Definitionen eines Typenmodells nur Themen und Klassen, die im Bezug zu Erweiterungen der vordefinierten Klasse `SIGN` stehen, sowie Laufzeitparameter deklarieren (vgl. <<_darstellungsbeschreibungen>> und <<_laufzeitparameter>>). Die Einhaltung dieser Regel ist ebenfalls Sache des Anwenders.
...
Mit der Angabe `NOINCREMENTALTRANSFER` wird definiert, dass beim Datentransfer (vgl. <<_sequentieller_transfer>>) kein inkrementeller Transfer unterstützt werden muss.footnote:[Das ist keine konzeptionelle Festlegung, zur Vereinfachung wird sie hier aber ermöglicht.] Insbesondere enthält das aus dem Modell generierte XML-Schema keine entsprechenden Möglichkeiten.

Anschliessend wird mittels Angabe des entsprechenden URI (vgl. <<_zeichenketten>>) der Herausgeber des Modells identifiziert. Es wird erwartet, dass der Modellname in diesem Kontext eindeutig ist.

++++
<a href="#3_5_C1">&#x203B</a>
<div id="2_5_C1"></div>
++++
[#3_5_C1]
.Syntaxregel:
...
Wird eine einzelne Klasse und nicht ein ganzes Thema geerbt, dürfen keine Beziehungen (vgl. <<_eigentliche_beziehungen>>) zu ihr definiert sein.
...
----
<1> Im PDF-Dokument des Referenzhandbuchs (Version 2.1.0) steht an dieser Stelle ClassOrStructureRef. In INTERLIS 2.4 dürfen aber Strukturen nicht mehr zu Klassen erweitert werden. Siehe dazu auch https://github.com/geostandards-ch/doc_refhb24/issues/16[dieses] Issue.

Welche Namen qualifiziert werden müssen (durch Model-Name bzw. durch Model-Name.Topic-Name) ist am Schluss des folgenden Abschnitts (vgl. <<_namensräume>>) erklärt. Klassen und Strukturen, die nicht auf einer bereits definierten Klasse oder Struktur aufbauen, brauchen keinen EXTENDS-Teil.
...
Metadaten-Behälter-Namen eröffnen den Zugang zu den Metaobjekten (vgl. <<_umgang_mit_metaobjekten>>).
