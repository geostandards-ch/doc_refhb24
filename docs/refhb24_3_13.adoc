...
* Numerische Ausdrücke (inkl. Koordinaten) sollen immer mit voller numerischer Genauigkeit (gemäss Norm IEEE-754) berechnet werden. Erst die resultierenden Werte sollen allenfalls gerundet werden (vgl. <<_umgang_mit_rundung_von_numerischen_werten_und_koordinaten>>).
...
* `THISAREA` und `THATAREA`: Bezeichnen die beiden Gebietsobjekte, auf dessen gemeinsamen Rand sich das aktuelle Linienzugsobjekt befindet. Der Einsatz von `THISAREA` und `THATAREA` ist nur möglich im Rahmen der Inspektion einer Gebietseinteilung (vgl. <<_sichten>>).

* `PARENT`: Bezeichnet das Ober-Strukturelement oder Ober-Objekt des aktuellen Strukturelementes oder Objektes. Die Sicht muss dazu eine gewöhnliche Inspektion (keine Gebietsinspektion) sein (vgl. <<_sichten>>).
...
* Das implizite Attribut `AGGREGATES` ist in Aggregationssichten (vgl. <<_sichten>>) definiert und bezeichnet den Satz (`BAG OF`) der aggregierten Basisobjekte.
...
* Wird mit dem Ausdruck eine Rollenangabe gemacht, bezeichnet der Ausdruck die Menge der über die Rolle verbundenen Zielobjekte. Beim formalen Parameter muss `OBJECT OF` oder `OBJECTS OF` (nur wenn auf Grund der Modellbeschreibung klar ist, dass nur ein Zielobjekt möglich ist) verlangt sein (vgl. <<_funktionen>>).

* Alle Objekte (`ALL`) der Klasse in deren Kontext der Funktionsaufruf erfolgt oder alle Objekte der angegebenen Klasse. Beim formalen Parameter muss `OBJECTS OF` verlangt sein (vgl. <<_funktionen>>). Damit sind immer alle Objekte gemeint, die dieser Klasse oder ihren Erweiterungen entsprechen.

Als Vergleichswerte kommen Funktionsaufrufe, Laufzeitparameter (vgl. <<_darstellungsbeschreibungen>>) und Konstanten in Frage.
